...
class Game extends Component {
  ...
  render() {
    return (<div className="game">
      <div className="game-board">
        <Board ref={this.boardRef} game={this}/>
        <br/>
  ...
  makeMoveWithValue(x, y, val) {
    ...
    for (var iy=gridStartY; iy<gridStartY+3; iy++) {
      for (var ix=gridStartX; ix<gridStartX+3; ix++) {
        if ((x != ix) && (y != iy)) {
  ...
  getMove() {
    ...
    for (var i = 0; i < 9; i++) {
      if (this.gameValueRefs[i].current.className == this.CLASS_NAME_SEL)
        move = parseInt(this.gameValueRefs[i].current.innerText);
    }
